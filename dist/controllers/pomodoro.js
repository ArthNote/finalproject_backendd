...
exports.getFocusSessionStats = getFocusSessionStats;
 ...
 */
function saveFocusSession(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const focusSession = yield prisma_1.db.focusSession.create({
                data: {
            ...
            if (completed && type === "work") {
                ...
                yield gameProgress.awardPomodoroCompletionXP(session.user.id, duration);
                ...
                if (userProgress) {
                    ...
                    const now = new Date();
                    const daysSinceLastStreak = Math.floor((now.getTime() - lastStreak.getTime()) / (1000 * 60 * 60 * 24));
                    if (daysSinceLastStreak <= 1 && daysSinceLastStreak > 0) {
                        ...
                        yield prisma_1.db.userProgress.update({
                            ...
                            data: {
                                ...
                                longestStreak: Math.max(userProgress.streakDays + 1, userProgress.longestStreak),
                                lastStreakDate: now,
                            },
                        });
                    }
                    ...
                    const dailyFocusGoals = yield prisma_1.db.goal.findMany({
                        where: {
                    ...
                    for (const goal of dailyFocusGoals) {
                        ...
                        const sessionCountMatch = goal.title.match(/(\d+)\s*(pomodoro|session|timer)/i);
                        ...
                        if (!sessionCompleted) {
                            if (goal.title.toLowerCase().includes("minute") ||
                                ...
                                if (minutesMatch && minutesMatch[1]) {
                                    ...
                                    if (!isNaN(targetMinutes) && targetMinutes > 0) {
                                        ...
                                        newProgress = Math.min(100, goal.progress + (duration / targetMinutes) * 100);
                                    }
                                    else {
                            ...
                            else if (goal.title.toLowerCase().includes("pomodoro") ||
 ...
 */
function getFocusSessions(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const headers = (0, node_1.fromNodeHeaders)(req.headers);
        const session = yield auth_1.auth.api.getSession({
            headers: headers,
        ...
        try {
            ...
            const focusSessions = yield prisma_1.db.focusSession.findMany({
                where: {
                    userId: session.user.id,
                    createdAt: {
                        gte: startDate,
            ...
            const totalSessions = focusSessions.filter((s) => s.type === "work" && s.completed).length;
            ...
            return res.status(200).json({
                ...
                stats: {
                    totalWorkMinutes,
                    totalSessions,
                    streak: (userProgress === null || userProgress === void 0 ? void 0 : userProgress.streakDays) || 0,
                },
            });
        }
        catch (error) {
 ...
 */
function getFocusSessionStats(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const headers = (0, node_1.fromNodeHeaders)(req.headers);
        const session = yield auth_1.auth.api.getSession({
            headers: headers,
        ...
        try {
            ...
            const dailySessions = Array.from(sessionsMap.entries()).map(([date, data]) => ({
                date,
